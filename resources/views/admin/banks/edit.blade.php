...
<div class="max-w-2xl">
    ...
    <div class="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8">
        <h2 class="text-2xl font-bold text-white mb-6">Edit Bank</h2>
        ...
        <form action="{{ route('admin.banks.update', $bank) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="bank_name" id="bank_name" value="{{ old('bank_name', $bank->bank_name) }}" required
            ...
            <div>
                ...
                <input type="text" name="account_holder" id="account_holder" value="{{ old('account_holder', $bank->account_holder) }}" required
                    class="w-full px-4 py-3 rounded-xl border border-white/10 bg-white/5 text-white placeholder-gray-500 focus:border-[#1C4D8D] focus:ring-2 focus:ring-[#1C4D8D]/50 transition"
